...
class Grid(svox2.SparseGrid):
    def __init__(
        ...
        # TODO: white_bkgd needs to be set
        ...
        #     # In DVGO, a negative density is possible.
        #     # Voxels labeled with 0 densities in the preprocessing are to be discarded.
        #     # mask = density_volume.reshape([-1]) != 0.
        #     mask = density_volume.reshape([-1]) > -100000.
        # else:
        #     mask = density_volume.reshape([-1]) > self.opt.sigma_thresh
        mask = density_volume.reshape([-1]) > self.opt.sigma_thresh
        # mask = density_volume.reshape([-1]) > 0.01
        capacity = mask.sum()
...
@torch.no_grad()
def calc_color_metric3(grid, color_res, color_weight):
    gsz = grid._grid_size()
    delta = grid.opt.step_size / (grid._scaling * gsz)
    print(f'Delta: \033[1;31m{delta}\033[0m')
    delta = delta[0]
    ...
    weight = 1. - torch.exp(-density * delta)
    return torch.sum(
        weight * (color_res / (color_weight + 1e-9))
    ), \
           (color_res / (color_weight + 1e-9)), weight
