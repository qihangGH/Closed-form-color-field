...
class SparseGrid(nn.Module):
    ...
    def _volume_render_gradcheck_lerp(self, rays: Rays, return_raylen: bool = False):
        ...
        while good_indices.numel() > 0:
            ...
            pos[:, 0] = torch.clamp_max(pos[:, 0], gsz_cu[0].long() - 1)
            pos[:, 1] = torch.clamp_max(pos[:, 1], gsz_cu[0].long() - 1)
            pos[:, 2] = torch.clamp_max(pos[:, 2], gsz_cu[0].long() - 1)
            #  print('pym', pos, log_light_intensity)

            points_long = pos.to(torch.long)
            points_long.clamp_min_(0)
            points_long[:, 0] = torch.clamp_max(points_long[:, 0], gsz_cu[0].long() - 2)
            points_long[:, 1] = torch.clamp_max(points_long[:, 1], gsz_cu[0].long() - 2)
            points_long[:, 2] = torch.clamp_max(points_long[:, 2], gsz_cu[0].long() - 2)
            pos -= points_long
    ...
    def estimate_color(
        ...
        """
        the kernel is implemented in `svox2/csrc/differentiable_estimate_color_kernel.cu`
        @param mask_out: a mask of grid.density_data.data
        @param c2ws: camera extrinsics, camera to world, (N, 4, 4)
        @param w2cs: camera extrinsics, world to camera, (N, 4, 4)
        @param intrinsics: a list of camera intrinsics and NDC coefficients
        @param images: images for color estimation
        @return: estimated SH coefficients for masked voxels
        """
        color_estimated = None
        links_inverse = self.links_inverse[mask_out]
        if _C is not None and self.density_data.is_cuda:
            color_estimated = _C.estimate_color(
                self._to_cpp(), links_inverse, self.opt._to_cpp(),
                c2ws, w2cs, intrinsics, images)
        else:
    ...
    def estimate_color_metric(
            ...
            images: torch.Tensor):
        '''
        Estimate the color using the closed-form method
        the kernel is implemented in `svox2/csrc/estimate_color_metric_kernel.cu`
        @param c2ws: camera extrinsics, camera to world, (N, 4, 4)
        @param w2cs: camera extrinsics, world to camera, (N, 4, 4)
        @param intrinsics: a list of camera intrinsics and NDC coefficients
        @param images: images for color estimation
    ...
    def differentiable_estimate_color(
            self,
            mask_out: torch.Tensor,
            c2ws: torch.Tensor,
            w2cs: torch.Tensor,
        ...
        color_estimated = None
        links_inverse = self.links_inverse[mask_out]
        grad_store = torch.zeros([len(links_inverse), len(c2ws), 3 * self.basis_dim]).to(c2ws.device)
        if _C is not None and self.density_data.is_cuda:
            color_estimated = _C.differentiable_estimate_color(
                self._to_cpp(), links_inverse,
                self.opt._to_cpp(), c2ws, w2cs, intrinsics, images, grad_store)
        else:
            print('To be implemented using torch functions.')
        return color_estimated, grad_store
    ...
    def add_color_grad(
        ...
        grad_density, grad_sh, _, _ = self._get_data_grads()
        grad_holder = _C.GridOutputGrads()
        grad_holder.grad_density_out = grad_density
        grad_holder.grad_sh_out = grad_sh
        grad_holder.mask_out = self.sparse_grad_indexer
    ...
    def get_mask_out(
            self,
            rays: Rays,
            positive_density_only: bool = False,
            use_kernel: bool = True
    ):
        """
        mask the voxels that are used to render 'rays'
        the kernel is implemented in `svox2/csrc/differentiable_estimate_color_kernel.cu`
        @param rays: cf rays for render
        @param positive_density_only: if true, mask only the positive density
        @param use_kernel: if true, use the kernel
        @return: the mask of the grid.density_data.data
        """
        mask_out = torch.zeros(
            (self.density_data.size(0),), dtype=torch.bool, device=self.density_data.device)
        if use_kernel and _C is not None and mask_out.is_cuda:
            mask_out = _C.get_mask_out(
                self._to_cpp(),
                self.opt._to_cpp(),
                rays._to_cpp(),
