...
// used by the submitted manuscript
...
// c / ( 2 * pi (e^c - e^{-c}) ) ; x / (2 * np.pi * (np.exp(x) - np.exp(-x)))
// 0.1   0.07944499699768888    0.01 0.07957614527022912
// 0.125 0.07937061569422553    0.25 0.07875454426711323     0.5 0.07635596666002062
// 1     0.06771391313789567    2    0.0438822907955184      4   0.011664010699794013
// const float CONC = 2.f;
// const float VMF_COEF = 0.0438822907955184f;
...
// forward facing: intrinsics.size(1) == 9
...
//     float color_weight = 0.f;
//
//     __shared__ float store_ray_dir[COLOR_CUDA_THREADS][3];
//
//     typedef cub::BlockReduce<float, COLOR_CUDA_THREADS> BlockReduce;
//     __shared__ typename BlockReduce::TempStorage temp_storage;
//
...
//     #pragma unroll 3
//     for (int i = 0; i < 3; ++i) {
//         store_ray_dir[cam_id][i] = ray.dir[i];
//     }
//     __syncthreads();
//     // Calculate the probability density for each direction
//     float sum_exp = 0.f;
//     for (int i = 0; i < cam_num; i++) {
//         sum_exp += expf(
//             CONC * (ray.dir[0] * store_ray_dir[i][0] + ray.dir[1] * store_ray_dir[i][1] + ray.dir[2] * store_ray_dir[i][2])
//         );
//     }
//     float pdf = VMF_COEF * sum_exp / cam_num;
//
//     // Calculate the basis value using normalized ray directions in the ndc coordinate.
//     calc_sh(basis_dim, ray.dir, basis_value);
//
...
//     float weight_sum_as_den = weight_sum + 1e-9f;
// //     float scale_parameter = PAI4 / weight_sum_as_den;
//     float scale_parameter = 1.f / weight_sum_as_den;
//     float weighted_basis_color = 0.f;               // T_q * c_q * Y_l(q)
//     float weighted_basis_color_sum = 0.f;           // sum_{q=1}^V {T_q * c_q * Y(q)}
//     float record_weighted_basis = 0.f;              // T_q * Y_l(q)
//     // Estimate each spherical harmonic coefficient in turn.
//     for (size_t idx_sh = 0; idx_sh < basis_dim; ++idx_sh) {
//         record_weighted_basis = color_weight * basis_value[idx_sh] / pdf;
//         for (int i = 0; i < 3; ++i) {
//             weighted_basis_color = record_weighted_basis * color_observed[i];
//             __syncthreads();  // block-wide sync barrier to re-use shared mem safely
//             weighted_basis_color_sum = BlockReduce(temp_storage).Sum(weighted_basis_color, cam_num);
...
// //     }
//     // No occ
// //     scale_parameter = PAI4 / cam_num;
// //     for (size_t idx_sh = 0; idx_sh < basis_dim; ++idx_sh) {
// //         for (int i = 0; i < 3; ++i) {
// //             weighted_basis_color = basis_value[idx_sh] * color_observed[i];
// //             __syncthreads();  // block-wide sync barrier to re-use shared mem safely
// //             weighted_basis_color_sum = BlockReduce(temp_storage).Sum(weighted_basis_color, cam_num);
// //             if (cam_id == 0)
// //                 results[block_id][idx_sh + basis_dim * i] = scale_parameter * weighted_basis_color_sum;
