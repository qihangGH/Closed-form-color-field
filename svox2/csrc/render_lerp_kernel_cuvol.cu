...
__device__ __inline__ void trace_ray_cuvol_high_degree(
        const PackedSparseGridSpec& __restrict__ grid,
        SingleRaySpec& __restrict__ ray,
        const RenderOptions& __restrict__ opt,
        uint32_t channel_id,
        ...
        float* __restrict__ out_log_transmit) {
    if (ray.tmin > ray.tmax) {
        out[channel_id] = (grid.background_nlayers == 0) ? opt.background_brightness : 0.f;
        if (channel_id == 0 && out_log_transmit != nullptr) {
            *out_log_transmit = 0.f;
        }
        return;
    }
...
#pragma unroll 3
        ...
        if (sigma > opt.sigma_thresh) {
            float lane_color = trilerp_cuvol_one(
                            ...
                            grid.sh_data_dim,
                            ray.l, ray.pos,
                            channel_id * grid.basis_dim + lane_id);
            lane_color *= sphfunc_val[lane_id]; // bank conflict

            const float pcnt = ray.world_step * sigma;
            const float weight = _EXP(log_transmit) * (1.f - _EXP(-pcnt));
            log_transmit -= pcnt;

//             float lane_color_total = WarpReducef(temp_storage).Sum(lane_color);
            float lane_color_total = WarpReducef(temp_storage).Sum(lane_color, grid.basis_dim);
            outv += weight * fmaxf(lane_color_total + 0.5f, 0.f);  // Clamp to [+0, infty)
            if (_EXP(log_transmit) < opt.stop_thresh) {
                log_transmit = -1e3f;
                break;
...
__device__ __inline__ void trace_ray_cuvol_backward_high_degree(
        const PackedSparseGridSpec& __restrict__ grid,
        const float* __restrict__ grad_output,
        const float* __restrict__ color_cache,
        SingleRaySpec& __restrict__ ray,
    ...
    const float gout = grad_output[channel_id];
...
#pragma unroll 3
        ...
        if (sigma > opt.sigma_thresh) {
            float lane_color = trilerp_cuvol_one(
                            ...
                            grid.size[2],
                            grid.sh_data_dim,
                            ray.l, ray.pos, channel_id * grid.basis_dim + lane_id);
            float weighted_lane_color = lane_color * sphfunc_val[lane_id];

            const float pcnt = ray.world_step * sigma;
            const float weight = _EXP(log_transmit) * (1.f - _EXP(-pcnt));
            log_transmit -= pcnt;

            const float lane_color_total = WarpReducef(temp_storage).Sum(
                weighted_lane_color, grid.basis_dim) + 0.5f;
            float total_color = fmaxf(lane_color_total, 0.f);
            float color_in_01 = total_color == lane_color_total;
            total_color *= gout; // Clamp to [+0, infty)
            color_in_01 = __shfl_sync((1U << grid.basis_dim) - 1, color_in_01, 0);

            const float grad_common = weight * color_in_01 * gout;
            const float curr_grad_color = sphfunc_val[lane_id] * grad_common;

            if (grid.basis_type != BASIS_TYPE_SH) {
                // TODO: Now only support sh type.
                printf("Grid basis type must be SH.\n");
            }
            ...
            trilerp_backward_cuvol_one(grid.links, grads.grad_sh_out,
                    ...
                    grid.sh_data_dim,
                    ray.l, ray.pos,
                    curr_grad_color, channel_id * grid.basis_dim + lane_id);

            __shared__ float total_color_c1;
            __shared__ float total_color_c2;
            if (channel_id == 1 && lane_id == 0) {
                total_color_c1 = total_color;
            }
            if (channel_id == 2 && lane_id == 0) {
                total_color_c2 = total_color;
            }
            __syncthreads();
            total_color += total_color_c1 + total_color_c2;
...
__launch_bounds__(TRACE_RAY_CUDA_THREADS, MIN_BLOCKS_PER_SM)
__global__ void render_ray_kernel_high_degree(
        PackedSparseGridSpec grid,
        PackedRaysSpec rays,
        RenderOptions opt,
        torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> out,
        float* __restrict__ log_transmit_out = nullptr) {
    const int ray_id = blockIdx.x;
    const int channel_id = threadIdx.x >> 5;  // color channel id.
    const int lane_id = threadIdx.x & 0x1F;  // lane id in each color channel.

    if (lane_id >= grid.basis_dim)  // Bad, but currently the best way due to coalesced memory access
        return;

    __shared__ float sphfunc_val[25];
    __shared__ SingleRaySpec ray_spec;
    __shared__ typename WarpReducef::TempStorage temp_storage[NUM_COLOR_CHANNEL];
    ray_spec.set(rays.origins[ray_id].data(), rays.dirs[ray_id].data());
    calc_sphfunc(grid, lane_id,
                 ray_id,
                 ray_spec.dir,
                 sphfunc_val);
...
__launch_bounds__(TRACE_RAY_CUDA_THREADS, MIN_BLOCKS_PER_SM)
__global__ void render_ray_image_kernel_high_degree(
        PackedSparseGridSpec grid,
        PackedCameraSpec cam,
        RenderOptions opt,
        float* __restrict__ out,
...
__launch_bounds__(TRACE_RAY_BKWD_CUDA_THREADS, MIN_BLOCKS_PER_SM)
__global__ void render_ray_backward_kernel_high_degree(
    PackedSparseGridSpec grid,
    const float* __restrict__ grad_output,
    const float* __restrict__ color_cache,
    PackedRaysSpec rays,
    ...
    __shared__ float sphfunc_val[25];
    __shared__ float grad_sphfunc_val[25];
    __shared__ SingleRaySpec ray_spec;
    __shared__ typename WarpReducef::TempStorage temp_storage[NUM_COLOR_CHANNEL];
    ray_spec.set(rays.origins[ray_id].data(), rays.dirs[ray_id].data());
    const float vdir[3] = {ray_spec.dir[0], ray_spec.dir[1], ray_spec.dir[2]};
    if (lane_id < grid.basis_dim) {
        grad_sphfunc_val[lane_id] = 0.f;
    }
    calc_sphfunc(grid, lane_id,
                 ray_id,
                 vdir, sphfunc_val);
...
#pragma unroll 3
    ...
    __syncthreads();
    trace_ray_cuvol_backward_high_degree(
        grid,
        grad_out,
        color_cache + ray_id * 3,
        ray_spec,
        ...
        accum_out == nullptr ? nullptr : accum_out + ray_id,
        log_transmit_out == nullptr ? nullptr : log_transmit_out + ray_id);
    // TODO: Now only support high degree sh, which doesn't need shpfunc backward.
    // calc_sphfunc_backward(
    //              grid, lane_id,
    //              ray_id,
    //              vdir,
...
torch::Tensor volume_render_cuvol(SparseGridSpec& grid, RaysSpec& rays, RenderOptions& opt) {
    ...
    {
        if (grid.sh_data.size(1) < WARP_SIZE) {
            ...
            device::render_ray_kernel_high_degree<<<Q, WARP_SIZE * 3>>>(
                    grid, rays, opt,
                    // Output
                    results.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                    use_background ? log_transmit.data_ptr<float>() : nullptr);
...
torch::Tensor volume_render_cuvol_image(SparseGridSpec& grid, CameraSpec& cam, RenderOptions& opt) {
...

    {
        if (grid.sh_data.size(1) < WARP_SIZE) {
            ...
            device::render_ray_image_kernel_high_degree<<<Q, WARP_SIZE * 3>>>(
                    grid,
                    cam,
                    opt,
                    // Output
...
void volume_render_cuvol_backward(
...

    {
        if (grid.sh_data.size(1) < WARP_SIZE) {
            ...
            device::render_ray_backward_kernel_high_degree<<<Q, WARP_SIZE * 3>>>(
                        grid,
                        grad_out.data_ptr<float>(),
                        color_cache.data_ptr<float>(),
                        rays, opt,
